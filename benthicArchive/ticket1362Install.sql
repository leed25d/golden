alter table maa.ph_activities add (is_maa_time varchar2(1));
update maa.ph_activities set is_maa_time = 1 where activity_id in (20000094,20000096,20000100,20000104,20000108,20000110,20000114,20000118,20000116,20000120,20000122,20000124);
...
BEGIN
  ...
  INSERT INTO maa.ph_activities
    (activity_number, activity_display, activity, enabled, is_maa_time, sample,
     sample_help, sample_note, order_number, survey_type,
     activity_id, default_activity)
  VALUES
    (p_activity_number, p_activity_display, p_activity, p_enabled, p_is_maa_time, p_sample,
...
CREATE OR REPLACE
PACKAGE BODY MAA."PH_SURVEY_PKG" is
...
BEGIN
  SELECT SUM(time)
   ...
   WHERE h.activity_id = a.activity_id
     AND a.activity_id in (select activity_id from maa.ph_activities where is_maa_time=1)
